            ...
            <ul className="mt-4 divide-y divide-zinc-200 text-sm">
              {latestMaterials.map((item) => (
                <li key={item.id} className="flex items-center justify-between gap-3 py-2.5">
                  <Link href={`/courses/${item.course.id}`} className="truncate text-zinc-900 hover:underline">
                    {item.title}
                  </Link>
                  <span className="shrink-0 text-xs text-zinc-500">{formatDateRu(item.createdAt)}</span>
                </li>
              ))}
            </ul>
          )}
              ...
              {recentLoginEvents.map((event) => (
                <li key={event.id} className="flex items-center justify-between gap-3 py-2.5">
                  <span className="truncate text-zinc-900">
                    {event.user.name} <span className="text-zinc-500">({event.user.login})</span>
                  </span>
                  <span className="shrink-0 text-xs text-zinc-500">{formatDateTimeRu(event.createdAt)}</span>
                </li>
              ))}
            </ul>
          )}
...
async function HrHomePage({ userId, searchParams }: { userId: string; searchParams: HrHomeSearchParams }) {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div>
        <div>
          <h1 className="text-3xl font-semibold tracking-tight text-zinc-950">Главная HR</h1>
        </div>
      </div>
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold text-zinc-950">Сегодня требует внимания</h2>
            </div>
            <Link
              ...
              <div className="mt-5 grid gap-3 lg:grid-cols-2">
                {attentionRows.map(({ item, priority }) => (
                  <Link
                    ...
                    className="rounded-xl border border-zinc-200 bg-zinc-50/70 px-4 py-3 transition hover:border-zinc-300 hover:bg-white"
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      <PriorityBadge priority={priority} />
                    </div>
                    <div className="mt-2 text-sm font-semibold text-zinc-950">{getHrEventTitle(item, now)}</div>
      ...
      <section className="mt-6 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-zinc-950">Портфель опубликованных курсов</h2>
        ...
        <div className="mt-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h3 className="text-sm font-semibold text-zinc-900">Завершения по месяцам</h3>
            <Link href="/analytics?tab=courses" className="text-xs font-medium text-sky-700 hover:underline">
              Подробная аналитика
            </Link>
          </div>
          <div className="mt-4 grid grid-cols-3 gap-3 sm:grid-cols-6">
            {courseAnalytics.monthlyCompletions.map((bucket) => {
              ...
              return (
                <div key={bucket.monthKey} className="rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-3">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[11px] font-medium uppercase text-zinc-500">{bucket.label}</span>
                    <span className="text-sm font-semibold text-zinc-900">{bucket.count}</span>
                  </div>
                  <div className="mt-3 flex h-20 items-end rounded-lg bg-white px-2 py-2">
                    <div className="w-full rounded-t-md bg-sky-600" style={{ height: `${height}%` }} />
                  </div>
                </div>
              );
            })}
      ...
      <section className="mt-6 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-zinc-950">Курсы с рисками</h2>
            <p className="mt-1 text-sm text-zinc-600">
          ...
          <>
            <div className="mt-5 space-y-3 md:hidden">
              {courseRiskRows.map(({ row, tone }) => (
                <article key={`mobile-risk:${row.id}`} className="rounded-xl border border-zinc-200 bg-zinc-50 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <Link
                        href={`/courses/${row.id}/learners`}
                        className="text-sm font-semibold text-zinc-950 hover:text-sky-700 hover:underline"
                      >
                        {row.title}
                      </Link>
                      <p className="mt-1 text-xs text-zinc-500">{row.groupName}</p>
                    </div>
                  ...
                  <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <dt className="text-xs text-zinc-500">Записано</dt>
                      <dd className="mt-1 font-semibold text-zinc-900">{row.assignedLearnersCount}</dd>
                    </div>
                    <div>
                      <dt className="text-xs text-zinc-500">Завершили</dt>
                      <dd className="mt-1 font-semibold text-zinc-900">
                        {row.completedLearnersCount} ({formatPercent(row.completedLearnersPercent)}%)
                      </dd>
                    </div>
                    <div>
                      <dt className="text-xs text-zinc-500">Не начали</dt>
                      <dd className="mt-1 font-semibold text-zinc-900">{row.notStartedLearnersCount}</dd>
                    </div>
                    <div>
                      <dt className="text-xs text-zinc-500">Истек доступ</dt>
                      <dd className="mt-1 font-semibold text-zinc-900">{row.expiredAccessLearnersCount}</dd>
                    </div>
                  </dl>
                  ...
                  <div className="mt-4">
                    <div className="flex items-center justify-between gap-3 text-xs text-zinc-600">
                      ...
                      <span className="font-semibold text-zinc-900">{formatPercent(row.avgProgressPercent)}%</span>
                    </div>
                  ...
                  {courseRiskRows.map(({ row, tone }) => (
                    <tr key={row.id} className="border-t border-zinc-200 align-top text-zinc-700">
                      <td className="px-4 py-4">
                        <Link
                          href={`/courses/${row.id}/learners`}
                          className="font-medium text-zinc-950 hover:text-sky-700 hover:underline"
                        >
                          {row.title}
                        </Link>
                        <div className="mt-1 max-w-md truncate text-xs text-zinc-500">{row.groupName}</div>
                      </td>
                      <td className="px-4 py-4 font-medium text-zinc-900">{row.assignedLearnersCount}</td>
                      <td className="px-4 py-4">
                        {row.completedLearnersCount} ({formatPercent(row.completedLearnersPercent)}%)
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex justify-end gap-2">
                          <Link
                            href={`/courses/${row.id}/learners?access=active`}
                            className="rounded-lg border border-zinc-200 px-3 py-1.5 text-xs font-medium text-zinc-900 hover:bg-zinc-50"
                          >
                            Ученики
                          </Link>
                          <Link
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[340px_minmax(0,1fr)]">
        <aside className="space-y-6">
          <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-zinc-950">HR-уведомления</h2>
            ...
            <Link
              ...
              className="mt-5 inline-flex h-10 items-center justify-center rounded-xl border border-zinc-200 bg-white px-4 text-sm font-medium text-zinc-900 hover:bg-zinc-50"
            >
              Настроить правила
            </Link>
          </div>
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
            <h2 className="text-lg font-semibold text-zinc-950">Низкая активность</h2>
            <p className="mt-2 text-sm text-zinc-600">Ученики с самым долгим перерывом в курсах.</p>

            {lowActivityItems.length === 0 ? (
              <div className="mt-4 rounded-xl border border-dashed border-zinc-300 bg-zinc-50/70 p-4 text-sm text-zinc-600">
              ...
              <div className="mt-4 space-y-3">
                {lowActivityItems.map((item) => (
                  <Link
                    ...
                    href={`/courses/${item.courseId}/learners/${item.learnerId}`}
                    className="block rounded-xl border border-zinc-200 px-3 py-3 hover:bg-zinc-50"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="text-sm font-medium text-zinc-950">{item.learnerName}</div>
                        <div className="mt-1 line-clamp-2 text-xs text-zinc-500">{item.courseTitle}</div>
                      </div>
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
            <h2 className="text-lg font-semibold text-zinc-950">Динамика 7 дней</h2>
            <p className="mt-2 text-sm text-zinc-600">Сколько событий появлялось по датам.</p>
            <div className="mt-5 flex h-32 items-end gap-2">
              {trendItems.map((item) => {
                const height = Math.max(8, Math.round((item.count / maxTrendCount) * 100));
                return (
                  <div key={item.key} className="flex flex-1 flex-col items-center gap-2">
                    <div className="flex h-24 w-full items-end rounded-full bg-zinc-100">
                      <div
        ...
        <div
          id="hr-event-feed"
          className="hr-target-panel scroll-mt-24 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm"
        >
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="text-xl font-semibold text-zinc-950">Лента событий</h2>
              <p className="mt-2 text-sm text-zinc-600">
              ...
              >
                ...
                <span
                  className={`rounded-full px-2 py-0.5 text-xs ${
                    item.isActive ? "bg-white/15 text-white" : "bg-zinc-100 text-zinc-600"
                  }`}
                >
                  {item.count}
                </span>
          ...
          <form action="/#hr-event-feed" className="mt-4 grid gap-3 lg:grid-cols-[1fr_1fr_1fr_150px_auto]">
            ...
            <label className="block">
              <span className="mb-1 block text-xs font-medium text-zinc-500">Тип события</span>
              <select
                name="eventType"
                defaultValue={filters.eventType}
                className="h-10 w-full rounded-xl border border-zinc-300 bg-white px-3 text-sm text-zinc-800 outline-none ring-emerald-500 focus:ring-2"
              >
            ...
            <label className="block">
              <span className="mb-1 block text-xs font-medium text-zinc-500">Курс</span>
              <select
                name="courseId"
            ...
            <label className="block">
              <span className="mb-1 block text-xs font-medium text-zinc-500">Группа</span>
              <select
                name="groupId"
            ...
            <label className="block">
              <span className="mb-1 block text-xs font-medium text-zinc-500">Статус</span>
              <select
                name="status"
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-zinc-500">
            <span className="rounded-full bg-zinc-100 px-3 py-1">Найдено: {filteredItems.length}</span>
            {filters.priority === "urgent" ? (
              <span className="rounded-full bg-rose-50 px-3 py-1 text-rose-700">Срочные</span>
            ) : null}
            {selectedCourse ? <span className="rounded-full bg-sky-50 px-3 py-1 text-sky-700">{selectedCourse.title}</span> : null}
            {selectedGroup ? <span className="rounded-full bg-emerald-50 px-3 py-1 text-emerald-700">{selectedGroup.name}</span> : null}
          </div>
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <h3 className="mt-3 text-lg font-semibold text-zinc-950">{getHrEventTitle(item, now)}</h3>
                        <p className="mt-2 text-sm leading-6 text-zinc-600">
                          {getHrEventDescription(item, notifications.preferences)}
                        </p>
                      </div>
                      <div className="text-xs text-zinc-500">{formatDateTimeRu(item.occurredAt)}</div>
                    </div>

                    <dl className="mt-4 grid gap-3 rounded-xl bg-zinc-50 px-4 py-3 text-sm sm:grid-cols-2">
                      <div>
                        <dt className="text-xs text-zinc-500">Ученик</dt>
                        <dd className="mt-1 font-medium text-zinc-900">
                          {item.learnerName} <span className="text-zinc-500">({item.learnerLogin})</span>
                        </dd>
                      </div>
                      <div>
                        <dt className="text-xs text-zinc-500">Курс</dt>
                        <dd className="mt-1 font-medium text-zinc-900">{item.courseTitle}</dd>
                      </div>
                    </dl>
...
function HrQuickActionLink({
  ...
  return (
    <Link
      href={href}
      className="group flex items-center justify-between gap-3 rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 hover:border-zinc-300 hover:bg-white"
    >
      <span>
...
function getHrToneClassName(tone: HrTone, intensity: "soft" | "strong") {
  if (intensity === "strong") {
    ...
    if (tone === "rose") return "bg-rose-600 text-white";
...
function MetricCard({
  ...
  return (
    <div className="rounded-xl border border-slate-700 bg-white p-5 shadow-sm">
      <div className="flex items-start gap-3">
        <div className="mt-0.5 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-emerald-700">
...
function PanelCard({
  ...
  return (
    <div className="min-h-[320px] rounded-xl border border-slate-700 bg-white p-5 shadow-sm">
      <div>
...
function EmptyState({ message }: { message: string }) {
  return <p className="mt-4 rounded-lg border border-dashed border-zinc-300 p-4 text-sm text-zinc-600">{message}</p>;
}
