...
export function CourseStructureSection(props: {
  ...
  return (
    <div className="space-y-8">
      ...
      {props.savedMessage ? (
        <p className="rounded-xl border border-[var(--success)] bg-[var(--success-soft)] px-4 py-3 text-sm text-[var(--success)]">{props.savedMessage}</p>
      ) : null}
      {props.errorMessage ? (
        <p className="rounded-xl border border-[var(--danger)] bg-[var(--danger-soft)] px-4 py-3 text-sm text-[var(--danger)]">{props.errorMessage}</p>
      ) : null}

      {!hasContent ? (
        <div className="rounded-xl border border-dashed border-[var(--line)] bg-[var(--surface)] p-8 text-center">
          <p className="text-sm text-[var(--ink-muted)]">Пока нет разделов и материалов. Добавьте первый раздел или учебный материал.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {props.moduleGroups.map((group, moduleIndex) => (
            <section key={group.id ?? `ungrouped-${moduleIndex}`} className="rounded-2xl border border-[var(--line)] bg-[var(--surface-raised)]">
              <div className="border-b border-[var(--line)] px-6 py-4">
                <p className="text-xs uppercase tracking-wide text-[var(--ink-muted)]">
                  {group.isSynthetic ? "Без раздела" : `Раздел ${moduleIndex + 1}`}
                </p>
                <div className="mt-1 flex items-center justify-between gap-3">
                  {!group.isSynthetic ? <h2 className="text-2xl font-semibold text-[var(--ink)]">{group.title}</h2> : null}
                  {!group.isSynthetic ? (
                    <Link href={`/courses/${courseId}/manage?section=structure&editModule=${group.id}`} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-[var(--line)] bg-[var(--surface-raised)] text-[var(--ink-muted)] hover:bg-[var(--accent-soft)]" aria-label="Редактировать раздел" title="Редактировать раздел">
                      <Pencil className="h-4 w-4" aria-hidden="true" />
                    </Link>
                  ) : null}
                </div>
                {group.description ? <p className="mt-1 text-sm text-[var(--ink-muted)]">{group.description}</p> : null}
              </div>
              {group.items.length === 0 ? (
                <p className="px-6 py-5 text-sm text-[var(--ink-muted)]">В этом разделе пока нет материалов.</p>
              ) : (
                <div className="px-6 py-2">
                  {group.items.map((item, itemIndex) => (
                    <div key={item.id} className="flex items-center gap-3 rounded-xl border-b border-[var(--line)] px-1 py-2.5 last:border-b-0 hover:bg-[var(--accent-soft)]">
                      <Link href={item.type === "QUIZ" && item.quiz ? `/courses/${courseId}/quiz/${item.quiz.id}/builder` : item.type === "SURVEY" ? `/courses/${courseId}/survey/${item.id}/builder` : `/courses/${courseId}/manage?section=structure&editItem=${item.id}`} className="flex min-w-0 flex-1 items-start gap-3">
                        <div className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-[var(--line)] bg-[var(--surface)] text-[var(--ink-muted)]">{getItemIcon(item.type)}</div>
                        <div className="min-w-0">
                          <h3 className="truncate text-[15px] font-semibold text-[var(--ink)]">{item.title}</h3>
                          <p className="text-xs text-[var(--ink-muted)]">{getItemTypeLabel(item)}</p>
                        </div>
                      </Link>
                      <details className="relative shrink-0" data-course-item-actions>
                        <summary className="inline-flex h-8 w-8 cursor-pointer list-none items-center justify-center rounded-lg border border-transparent text-[var(--ink-muted)] hover:border-[var(--line)] hover:bg-[var(--surface-raised)] hover:text-[var(--ink)]" aria-label="Действия с материалом" title="Действия"><MoreVertical className="h-4 w-4" aria-hidden="true" /></summary>
                        <div className="absolute right-0 top-9 z-20 w-44 rounded-xl border border-[var(--line)] bg-[var(--surface-raised)] p-1 shadow-lg">
                          <form action={moveCourseItem.bind(null, courseId, item.id, "up")}><button type="submit" disabled={itemIndex === 0} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-[var(--ink)] hover:bg-[var(--accent-soft)] disabled:cursor-not-allowed disabled:opacity-40"><ArrowUp className="h-4 w-4" aria-hidden="true" />Переместить выше</button></form>
                          <form action={moveCourseItem.bind(null, courseId, item.id, "down")}><button type="submit" disabled={itemIndex === group.items.length - 1} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-[var(--ink)] hover:bg-[var(--accent-soft)] disabled:cursor-not-allowed disabled:opacity-40"><ArrowDown className="h-4 w-4" aria-hidden="true" />Переместить ниже</button></form>
                        </div>
                      </details>
                    </div>
                  ))}
...
function ItemDialog(props: Parameters<typeof CourseStructureSection>[0] & { item: StructureItem }) {
  ...
  return (
    <div className="admin-content-modal fixed z-40 flex items-center justify-center bg-black/40 p-4">
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="courseItemDialogTitle" className="relative z-10 max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-[var(--line)] bg-[var(--surface-raised)] p-6 shadow-2xl">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div><p className="text-xs uppercase tracking-wide text-[var(--ink-muted)]">{getItemTypeLabel(item)}</p><h3 id="courseItemDialogTitle" className="text-xl font-semibold text-[var(--ink)]">{item.title}</h3></div>
          <div className="flex flex-wrap items-center gap-2">
            <Link href={props.selectedItemPreviewHref ?? `/courses/${courseId}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-[var(--line)] px-3 py-1.5 text-sm text-[var(--ink)] hover:bg-[var(--accent-soft)]"><Eye className="h-4 w-4" aria-hidden="true" />Предпросмотр</Link>
            <Link href={`/courses/${courseId}/manage?section=structure`} className="rounded-lg border border-[var(--line)] px-3 py-1.5 text-sm text-[var(--ink)] hover:bg-[var(--accent-soft)]">Закрыть</Link>
          </div>
        </div>
...
function ModuleDialog({ courseId, courseModule }: { courseId: string; courseModule: StructureModule }) {
  return <div className="admin-content-modal fixed z-40 flex items-center justify-center bg-black/40 p-4"><CourseModalAutoClose href={`/courses/${courseId}/manage?section=structure`} /><Link href={`/courses/${courseId}/manage?section=structure`} aria-label="Закрыть окно редактирования раздела" className="absolute inset-0" /><div role="dialog" aria-modal="true" aria-labelledby="courseModuleDialogTitle" className="relative z-10 w-full max-w-2xl rounded-2xl border border-[var(--line)] bg-[var(--surface-raised)] p-6 shadow-2xl"><div className="mb-4 flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-wide text-[var(--ink-muted)]">Раздел</p><h3 id="courseModuleDialogTitle" className="text-xl font-semibold text-[var(--ink)]">{courseModule.title}</h3></div><Link href={`/courses/${courseId}/manage?section=structure`} className="rounded-lg border border-[var(--line)] px-3 py-1.5 text-sm text-[var(--ink)] hover:bg-[var(--accent-soft)]">Закрыть</Link></div><form action={updateCourseModule.bind(null, courseId, courseModule.id)} className="grid gap-3"><input name="title" defaultValue={courseModule.title} className="rounded-xl border border-[var(--line)] bg-[var(--surface-raised)] px-3 py-2 text-sm outline-none ring-[var(--accent)] focus:ring-2" /><textarea name="description" rows={3} defaultValue={courseModule.description ?? ""} placeholder="Описание раздела" className="rounded-xl border border-[var(--line)] bg-[var(--surface-raised)] px-3 py-2 text-sm outline-none ring-[var(--accent)] focus:ring-2" /><div><button type="submit" className="rounded-xl border border-[var(--line)] px-4 py-2 text-sm hover:bg-[var(--accent-soft)]">Сохранить раздел</button></div></form><form action={deleteCourseModule.bind(null, courseId, courseModule.id)} className="mt-3"><button type="submit" className="text-sm text-[var(--danger)] underline">Удалить раздел</button></form></div></div>;
}

function BuilderLink({ href, label }: { href: string; label: string }) {
  return <div className="mt-6 space-y-6 border-t border-[var(--line)] pt-6"><Link href={href} className="rounded-xl bg-[var(--accent)] px-3 py-2 text-sm font-medium text-white hover:bg-[var(--accent-strong)]">{label}</Link></div>;
}
...
function SectionIntro({ title, description, aside, actions }: { title: string; description: string; aside: string; actions: ReactNode }) {
  return <div className="space-y-2"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-semibold text-[var(--ink)]">{title}</h2><div className="flex flex-wrap items-center justify-start gap-3 sm:justify-end"><span className="rounded-full border border-[var(--line)] bg-[var(--surface)] px-3 py-1.5 text-sm text-[var(--ink-muted)]">{aside}</span>{actions}</div></div><p className="max-w-3xl text-sm text-[var(--ink-muted)]">{description}</p></div>;
}
