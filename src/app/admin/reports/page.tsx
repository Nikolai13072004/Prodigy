  ...
  const reportRows = [
    {
      ...
      metricClassName: "bg-[var(--success-soft)] text-[var(--success)]",
    ...
    },
    {
      ...
      metricClassName: "bg-[var(--info-soft)] text-[var(--info)]",
    ...
    },
    {
      ...
      metricClassName: "bg-[var(--warning-soft)] text-[var(--warning)]",
    ...
    },
    {
      ...
      metricClassName: "bg-[var(--danger-soft)] text-[var(--danger)]",
  ...
  return (
    <main className="mx-auto max-w-6xl">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight text-[var(--ink)]">Отчеты HR</h1>
      ...
      <AnalyticsSubtabs active="reports" />
        ...
        <div className="divide-y divide-[var(--line)]">
          {reportRows.map((report) => (
            <Link
              ...
              className="grid gap-4 px-5 py-5 transition hover:bg-[var(--accent-soft)] sm:px-6 lg:grid-cols-[minmax(0,1fr)_150px] lg:items-center"
            >
              <div className="min-w-0">
                <h3 className="text-lg font-semibold text-[var(--ink)]">{report.title}</h3>
                <p className="mt-1 max-w-3xl text-sm leading-6 text-[var(--ink-muted)]">{report.description}</p>
              </div>
              ...
              <div className="flex items-center gap-3 lg:justify-end">
                ...
                <span className="text-sm font-medium text-[var(--accent)]">Открыть «{report.title}»</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section id="hr-notifications" className="mt-6 rounded-2xl border border-[var(--line)] bg-[var(--surface-raised)] p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl font-semibold text-[var(--ink)]">HR-уведомления</h2>
            <p className="mt-2 max-w-3xl text-sm text-[var(--ink-muted)]">
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
          <form action={saveHrNotificationPreferences} className="space-y-4 rounded-xl border border-[var(--line)] bg-[var(--surface)] p-4">
            <input type="hidden" name="returnTo" value="/admin/reports#hr-notifications" />

            <label className="flex items-start gap-3 rounded-xl border border-[var(--line)] bg-[var(--surface-raised)] px-4 py-3">
              <input
                type="checkbox"
              ...
              <span>
                ...
                <span className="mt-1 block text-xs text-[var(--ink-muted)]">Показывать завершения курсов за последние 30 дней.</span>
              </span>
            </label>
            ...
            <label className="flex items-start gap-3 rounded-xl border border-[var(--line)] bg-[var(--surface-raised)] px-4 py-3">
              ...
              <span>
                ...
                <span className="mt-1 block text-xs text-[var(--ink-muted)]">Показывать учеников, которые давно не заходили в курс.</span>
              </span>
            </label>
            ...
            <label className="flex items-start gap-3 rounded-xl border border-[var(--line)] bg-[var(--surface-raised)] px-4 py-3">
              ...
              <span>
                ...
                <span className="mt-1 block text-xs text-[var(--ink-muted)]">Показывать учеников, у которых скоро закончится доступ.</span>
              </span>
            </label>

            <label className="block">
              <span className="mb-2 block text-sm font-medium text-[var(--ink)]">Предупреждать об истечении доступа за, дней</span>
              <input
            ...
            <button
              type="submit"
              className="w-full rounded-xl bg-[var(--accent)] px-4 py-2.5 text-sm font-medium text-white hover:bg-[var(--accent-strong)]"
            >
              Сохранить настройки
            </button>
          </form>
          ...
          <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-raised)] p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h3 className="text-lg font-semibold text-[var(--ink)]">Текущие события</h3>
              ...
              <form action={queueMyHrNotificationEmails}>
                ...
                <button
                  type="submit"
                  className="rounded-xl border border-[var(--line)] bg-[var(--surface-raised)] px-4 py-2.5 text-sm font-medium text-[var(--ink)] hover:bg-[var(--accent-soft)]"
                >
            ...
            {notifications.items.length === 0 ? (
              ...
              <div className="mt-5 space-y-3">
                {notifications.items.map((item) => (
                  <article key={item.notificationKey} className="rounded-xl border border-[var(--line)] px-4 py-4">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <Link href={item.href} className="text-sm font-semibold text-[var(--ink)] hover:text-[var(--accent)] hover:underline">
      ...
      <section id="report-schedules" className="mt-6 grid gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
        <div className="rounded-2xl border border-[var(--line)] bg-[var(--surface-raised)] p-5 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-[var(--ink)]">Расписания отчетов</h2>
              <p className="mt-2 text-sm text-[var(--ink-muted)]">
                Ежемесячная рассылка формируется 1-го числа в 09:00. Получатели получают email-сводку и ссылку на отчет
                в LMS.
              </p>
            </div>
            <div className="rounded-full bg-[var(--surface)] px-3 py-1 text-xs font-medium text-[var(--ink-muted)]">
              {activeScheduleCount} активных
            </div>
          </div>

          {sp.scheduleNotice ? (
            <div className="mt-4 rounded-xl border border-[var(--success)] bg-[var(--success-soft)] px-4 py-3 text-sm text-[var(--success)]">
              {sp.scheduleNotice}
            </div>
          ) : null}

          {sp.scheduleError ? (
            <div className="mt-4 rounded-xl border border-[var(--danger)] bg-[var(--danger-soft)] px-4 py-3 text-sm text-[var(--danger)]">
              {sp.scheduleError}
            </div>
          ) : null}
          ...
          <form action={createHrReportSchedule} className="mt-5 space-y-4">
            ...
            <label className="block">
              <span className="mb-1 block text-sm font-medium text-[var(--ink)]">Получатели</span>
              <textarea
                name="recipients"
                rows={5}
                placeholder={"lead@example.com\nowner@example.com"}
                className="w-full rounded-xl border border-[var(--line)] px-3 py-2 text-sm outline-none ring-[var(--accent)] focus:ring-2"
              />
            ...
            <div className="rounded-xl border border-[var(--line)] bg-[var(--surface)] px-4 py-3 text-sm text-[var(--ink-muted)]">
        ...
        <div className="rounded-2xl border border-[var(--line)] bg-[var(--surface-raised)] p-5 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-[var(--ink)]">Текущие расписания</h2>
              <p className="mt-2 text-sm text-[var(--ink-muted)]">
...
function ReportScheduleCard({ schedule }: { schedule: HrReportScheduleView }) {
  return (
    <div className="rounded-xl border border-[var(--line)] px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm font-semibold text-[var(--ink)]">{schedule.reportTypeLabel}</span>
            <ReportScheduleStatusBadge isPaused={schedule.isPaused} />
          </div>
          <p className="mt-2 text-sm text-[var(--ink-muted)]">Получатели: {schedule.recipients.join(", ")}</p>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <form action={deleteHrReportSchedule.bind(null, schedule.id)}>
            <button
              type="submit"
              className="rounded-xl border border-[var(--danger)] bg-[var(--surface-raised)] px-3 py-2 text-sm font-medium text-[var(--danger)] hover:bg-[var(--danger-soft)]"
            >
              Удалить
            </button>
          </form>
  ...
  return (
    <span
      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${
        isPaused ? "bg-[var(--warning-soft)] text-[var(--warning)]" : "bg-[var(--success-soft)] text-[var(--success)]"
      }`}
    >
...
function getHrNotificationTone(type: HrNotificationItem["type"]) {
  if (type === "course_completed") return "bg-[var(--success-soft)] text-[var(--success)]";
  if (type === "low_activity") return "bg-[var(--warning-soft)] text-[var(--warning)]";
