...
import { USER_STATUSES, buildUserDisplayName, isEditableUserStatus } from "@/lib/users";
...
export async function createUser(formData: FormData) {
  ...
  const submitMode = canEditAccessLevel
    ? asString(formData, "submitMode")
    : "invite";
  const shouldSendInvite = canEditAccessLevel
    ? submitMode === "invite"
    : hasCheckedValue(formData, "sendInvite", "1");
  ...
  const passwordInput = canEditAccessLevel
    ? asString(formData, "password")
    : "";
  const password = canEditAccessLevel
    ? shouldSendInvite
      ? generatePasswordForPolicy(securitySettings)
      : passwordInput
    : generatePasswordForPolicy(securitySettings);
  const roles = canEditAccessLevel
    ? collectRoleNames(formData)
  ...
  const activationToken =
    canEditAccessLevel && shouldSendInvite ? createUserActivationToken() : null;
  let createdUser: {
    id: string;
    email: string | null;
    login: string;
...
export async function bulkArchiveUsers(formData: FormData) {
  ...
  const skippedCurrentUser = users.some((user) => user.id === session.user.id);
  const archiveUserIds = users
    .filter(
      (user) =>
        user.id !== session.user.id && user.status !== USER_STATUSES.ARCHIVED,
    )
    .map((user) => user.id);
...
export async function updateUser(userId: string, formData: FormData) {
  ...
  if (canEditAccessLevel) {
    if (statusControl === "activity-toggle") {
      const activeUser = hasCheckedValue(formData, "activeUser", "1");
      if (currentUser.status === USER_STATUSES.ARCHIVED) {
        status = currentUser.status;
      } else if (!activeUser) {
        status = USER_STATUSES.BLOCKED;
      } else if (currentUser.status === USER_STATUSES.BLOCKED) {
        status = USER_STATUSES.ACTIVE;
      }
    } else if (isEditableUserStatus(statusRaw)) {
      status = statusRaw;
    }
  }
  ...
  if (
    ...
    status === USER_STATUSES.BLOCKED &&
    currentUser.status !== USER_STATUSES.BLOCKED
  ) {
  ...
  const rolesChanged =
    ...
    currentRoles.some((currentRole) => !roles.includes(currentRole));
