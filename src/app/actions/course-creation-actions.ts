...
import { getCourseTemplateLabel, isCourseTemplateKey } from "@/lib/course-creation-options";
import {
  getCourseTemplateBlueprint,
  planTemplateCourseItems,
  type TemplateItemBlueprint,
} from "@/modules/course/domain/course-template-blueprint";
...
async function resolvePresentationFileUrlFromForm(
  ...
  for (const candidate of presentationFileUrlCandidates(presentationPreviewUrl, presentationViewMode)) {
...
async function createTemplateItems(
  ...
  for (const spec of planTemplateCourseItems(moduleIds, items)) {
