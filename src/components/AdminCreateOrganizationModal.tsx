...
export function AdminCreateOrganizationModal({ action }: Props) {
  ...
  return (
    <>
      <Button onClick={() => setOpen(true)}>Новая организация</Button>

      {open ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4">
          <div className="w-full max-w-md rounded-lg bg-[var(--surface-raised)] shadow-xl">
            <div className="flex items-center justify-between border-b border-[var(--line)] px-5 py-3">
              <h3 className="text-xl font-medium text-[var(--ink)]">Новая организация</h3>
              <button
                type="button"
                aria-label="Закрыть"
            ...
            >
              <Label htmlFor="new-organization-name" className="mb-2 block">
                Название
              </Label>
              <input
