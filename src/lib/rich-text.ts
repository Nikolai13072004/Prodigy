...
export function sanitizeRichTextHtml(value: string | null | undefined) {
  ...
  // Незавершённый тег в конце (без закрывающей '>') переживает перезапись выше:
  // регулярка требует '>' и такой хвост не матчит. Сам по себе он инертен —
  // браузер отбрасывает тег, оборванный концом ввода. Но результат санитайзера
  // не всегда попадает в DOM целиком: buildPlatformHtmlEmailFromRichHtml
  // (src/lib/email/template-settings.ts) вставляет его в `<td ...>${content}</td>`,
  // и тогда '>' приходит из соседней разметки, оживляя обрывок. Санитайзер обязан
  // возвращать безопасный результат независимо от контекста вставки.
  sanitized = sanitized
    .replace(/<\/?[a-z][^>]*$/i, "")
